...
public class Torrent {
    ...
    public Torrent(byte[] torrent, boolean seeder) throws IOException, NoSuchAlgorithmException {
        ...
        this.creationDate = this.decoded.containsKey("creation date") ? new Date(this.decoded.get("creation date")
                .getLong() * 1000) : null;
        ...
        logger.info("  Pieces......: {} piece(s) ({} byte(s)/piece)", (this.size / this.decoded_info
                .get("piece length").getInt()) + 1, this.decoded_info.get("piece length").getInt());
     ...
     *            creator.
     */
    public static Torrent create(File source, URI announce, String createdBy) throws NoSuchAlgorithmException,
    ...
    private static Torrent create(File parent, List<File> files, URI announce, List<List<URI>> announceList,
            String createdBy) throws NoSuchAlgorithmException, InterruptedException, IOException {
     ...
     *            The file to hash.
     */
    private static String hashFile(File file) throws NoSuchAlgorithmException, InterruptedException, IOException {
        return Torrent.hashFiles(Arrays.asList(new File[] { file }));
    }

    private static String hashFiles(List<File> files) throws NoSuchAlgorithmException, InterruptedException,
            IOException {
        ...
        for (File file : files) {
            logger.info("Hashing data from {} with {} threads ({} pieces)...", new Object[] { file.getName(), threads,
