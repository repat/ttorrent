...
public class Announce implements Runnable {
     ...
     * @param type
     *            A string representing the announce type (used in the thread
     *            name).
     */
    public Announce(SharedTorrent torrent, Peer peer) {
        ...
        for (List<URI> tier : torrent.getAnnounceList()) {
            ...
            for (URI tracker : tier) {
                try {
                    ...
                    logger.warn("Will not announce on {}: {}!", tracker, e.getMessage() != null ? e.getMessage() : e
        ...
        logger.info("Initialized announce sub-system with {} trackers on {}.", new Object[] {
                torrent.getTrackerCount(), torrent });
    }
    ...
    private void promoteCurrentTrackerClient() {
        logger.trace("Promoting current tracker client for {} " + "(tier {}, position {} -> 0).", new Object[] {
